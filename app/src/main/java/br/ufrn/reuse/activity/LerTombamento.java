...
public class LerTombamento extends AbstractActivity implements ZXingScannerView.ResultHandler {
        ...
        switch (requestCode) {
            case REQUEST_CAMERA:
                if (grantResults.length > 0) {
                    ...
                    if (cameraAccepted){
                        Toast.makeText(getApplicationContext(), "Permissão Aceita", Toast.LENGTH_LONG).show();
                    }else {
    ...
    private void showMessageOKCancel(String message, DialogInterface.OnClickListener okListener) {
        new android.support.v7.app.AlertDialog.Builder(LerTombamento.this)
                .setMessage(message)
    ...
    public void handleResult(Result result) {
        ...
        tombamento_g = myResult;

        Intent intent = new Intent();
        intent.putExtra("resultado", myResult);
